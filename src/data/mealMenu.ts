/**
 * "התפריט שלי" — הרובריקה שמעל החיפוש במסך התזונה. ארבע קבוצות בסדר קבוע,
 * ורק פריטי ספרייה (c:lib2:*). ה-slug הוא מזהה הפריט בספרייה; `grams` הוא
 * מה שנרשם בלחיצה אחת — למנה מורכבת, ברירת המחדל היא משקל המנה כפי
 * שהוגדרה (recipe.finalGrams), ולכן אין כאן מספר.
 *
 * הערכים עצמם לא כאן: הם נקראים מהמזון בספרייה בזמן ריצה.
 */

import type { MenuGroup } from '../lib/nutrition/menu';

/** אגוז אחד ביום, 20–25 ג'. המנה ברובריקה. */
export const NUT_GRAMS = 25;

export const MEAL_MENU: readonly MenuGroup[] = [
  {
    key: 'lunch',
    label: 'צהריים',
    items: [
      { slug: 'lunch-1-chicken', label: 'צ1 חזה עוף' },
      { slug: 'lunch-2-roastbeef', label: 'צ2 רוסטביף' },
      { slug: 'lunch-3-mixed', label: 'צ3 מעורב' },
      { slug: 'lunch-4-pastrami', label: 'צ4 פסטרמה' },
      { slug: 'lunch-5-tuna-eggs', label: 'צ5 טונה וביצים' },
    ],
  },
  {
    key: 'dinner',
    label: 'ערב',
    items: [
      { slug: 'dinner-1-cottage-eggs', label: "ע1 קוטג' וביצים" },
      { slug: 'dinner-2-shakshuka', label: 'ע2 שקשוקה' },
      { slug: 'dinner-3-eggs-cheese', label: 'ע3 ביצים וגבינה' },
      { slug: 'dinner-4-broccoli-pie', label: 'ע4 פשטידה (תבנית)' },
      { slug: 'dinner-5-no-cook', label: 'ע5 בלי בישול' },
    ],
  },
  {
    key: 'blocks',
    label: 'בלוקים',
    items: [
      { slug: 'tuna-water-drained', label: 'טונה', grams: 112 },
      { slug: 'greek-yogurt-0', label: 'יוגורט יווני', grams: 200 },
      { slug: 'block-cottage-5', label: "קוטג'", grams: 100 },
      { slug: 'bulgarit-5', label: 'בולגרית', grams: 100 },
      { slug: 'block-egg-whites', label: 'חלבוני ביצה', grams: 100 },
      { slug: 'pro40-yotvata', label: 'PRO 40', grams: 1 },
    ],
  },
  {
    key: 'extras',
    label: 'תוספות',
    items: [
      { slug: 'nut-almonds', label: 'שקדים', grams: NUT_GRAMS, nut: true },
      { slug: 'nut-walnuts', label: 'אגוזי מלך', grams: NUT_GRAMS, nut: true },
      { slug: 'nut-hazelnuts', label: 'אגוזי לוז', grams: NUT_GRAMS, nut: true },
      { slug: 'nut-pecans', label: 'פקאן', grams: NUT_GRAMS, nut: true },
      { slug: 'nut-peanuts', label: 'בוטנים', grams: NUT_GRAMS, nut: true },
      { slug: 'nut-pistachios', label: 'פיסטוקים', grams: NUT_GRAMS, nut: true },
      { slug: 'nut-cashews', label: 'קשיו', grams: NUT_GRAMS, nut: true },
      { slug: 'pita-light', label: 'פיתה קלה', grams: 1 },
      { slug: 'date-ball', label: 'כדור תמר', grams: 1 },
      { slug: 'coffee-milk', label: 'קפה עם חלב' },
    ],
  },
];
